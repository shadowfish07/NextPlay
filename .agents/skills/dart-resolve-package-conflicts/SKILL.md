...
Choose the Pub frontend before running any command in this guide. Use
`flutter pub` when `pubspec.yaml` declares `flutter: sdk: flutter`; use
`dart pub` for a pure Dart package. The command examples use `pub_sdk` so the
choice stays consistent:

```bash
# Choose exactly one for the current project.
pub_sdk=flutter
# pub_sdk=dart
```

Understand the output columns of `"$pub_sdk" pub outdated`:
*   **Current:** The version currently recorded in `pubspec.lock`.
*   **Upgradable:** The latest version allowed by the constraints in `pubspec.yaml`. `"$pub_sdk" pub upgrade` resolves to this.
...
*   **Enforce Lockfiles in CI:** Use `"$pub_sdk" pub get --enforce-lockfile` in CI/CD pipelines to ensure the exact versions tested locally are used in production.
...
- [ ] **If updating to "Upgradable" versions:**
  - [ ] Run `"$pub_sdk" pub upgrade`.
  - [ ] Run `"$pub_sdk" pub upgrade --tighten` to automatically update the lower bounds in `pubspec.yaml` to match the newly resolved versions.
...
**NEVER** delete the entire `pubspec.lock` file and run `"$pub_sdk" pub get`. This causes uncontrolled upgrades across the entire dependency graph.

**Task Progress:**
- [ ] Run `"$pub_sdk" pub upgrade --unlock-transitive <package>` for the affected package.
...
When `"$pub_sdk" pub outdated` shows a package is resolvable to a higher minor/patch version, use the `--tighten` flag to update the `pubspec.yaml` automatically.
...
**Command:**
```bash
"$pub_sdk" pub upgrade --tighten http
```
...
**Action:** Delete the `package_a` block entirely. Leave `package_b` untouched. Run `"$pub_sdk" pub get`.
