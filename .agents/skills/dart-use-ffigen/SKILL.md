...
*   **Clean-Checkout Availability**: Generated bindings must exist before any
    analyzer, test, build, or consumer imports them in a clean checkout. Choose
    one explicit strategy: invoke `tool/ffigen.dart` from the repository-owned
    setup/code-generation gate and CI, or commit the generated files with
    narrow `.gitignore` exceptions. Never rely on ignored local `*.g.dart`
    files. In NextPlay, `build_runner` does not execute `tool/ffigen.dart`, so
    wire FFIgen into the owned generation path separately if the outputs remain
    ignored.
*   **No Stale Bindings**: Run the generator script and inspect whether the
    generated files changed before finishing. Always verify the package with
    the repository gate (or `dart analyze` in a standalone Dart package).
...
Then make the outputs reproducible in a clean checkout:

- Prefer adding that command to the repository-owned setup/code-generation
  helper that runs before analysis, tests, and builds, and ensure CI invokes the
  same helper. For NextPlay, do not assume the existing `build_runner` command
  runs FFIgen.
- If the repository tracks generated bindings instead, add narrow exceptions
  for the exact outputs rather than removing the global generated-code rule:
  ```gitignore
...
3.  **Clean-Checkout Proof**: Confirm each imported generated output is either
    tracked (`git ls-files --error-unmatch <output>`) or produced by the
