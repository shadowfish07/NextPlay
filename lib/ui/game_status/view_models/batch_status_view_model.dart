...
class BatchStatusViewModel extends ChangeNotifier {
  ...
  void _initializeCommands() {
    // 初始化Command
    initializeCommand = Command.createAsyncNoParamNoResult(() async {
      ...
      try {
        ...
        AppLogger.info(
          'GameRepository has ${gameStatuses.length} game statuses',
        );

        if (gameLibrary.isEmpty) {
          AppLogger.warning(
            'Game library is empty, checking if data is being loaded...',
          );
          _setState(
            _state.copyWith(
              isLoading: false,
        ...
        // 筛选0时长游戏
        final zeroPlaytimeGames = _findZeroPlaytimeGames(
          gameLibrary,
          gameStatuses,
        );

        // 筛选高时长游戏
        final highPlaytimeGames = _findHighPlaytimeGames(
          gameLibrary,
          gameStatuses,
        );
        ...
        _setState(
          _state.copyWith(
            ...
            totalCount:
                zeroPlaytimeGames.length +
                highPlaytimeGames.length +
                abandonedGames.length,
          ),
        );

        AppLogger.info(
          'Batch status management initialized: ${zeroPlaytimeGames.length} zero-playtime games, ${highPlaytimeGames.length} high-playtime games, ${abandonedGames.length} abandoned games',
        );
      } catch (e, stackTrace) {
  ...
  List<GameSelectionItem> _findZeroPlaytimeGames(
    ...
    return gameLibrary.where((game) => game.playtimeForever == 0).map((game) {
      final currentStatus =
  ...
  List<GameSelectionItem> _findHighPlaytimeGames(
    ...
    return gameLibrary
        .where((game) {
          ...
          return hoursPlayed > game.estimatedCompletionHours * 0.8 &&
              hoursPlayed > 5.0;
        })
        .map((game) {
          final currentStatus =
          ...
          final completionRate =
              (hoursPlayed / game.estimatedCompletionHours * 100).toInt();
  ...
  List<GameSelectionItem> _findAbandonedGames(
    ...
    return gameLibrary
        .where((game) {
          ...
          if (hoursPlayed < game.estimatedCompletionHours * 0.5 &&
              hoursPlayed > 1.0) {
            // 检查最后游玩时间
  ...
  Future<void> _updateGameStatus(int appId, GameStatus status) async {
    ...
    try {
      ...
      if (result.isError()) {
        AppLogger.error(
          'Failed to update game status for appId $appId: ${result.exceptionOrNull()}',
        );
      } else {
        AppLogger.info(
          'Successfully updated game status for appId $appId to ${status.displayName}',
        );
      ...
      AppLogger.error(
        'Error updating game status for appId $appId',
        e,
        stackTrace,
      );
    }
